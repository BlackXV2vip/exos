...
- 🌐 **واجهة ويب مدمجة** — `exos-agent serve` وافتح المتصفح: واجهة كاملة بعلامتنا التجارية، من غير سيرفرات خارجية.
...
- **Embedded Web UI** (`exos-agent serve`) — full branded interface in your browser, zero external servers.
...
- 🖥️ **PC / لينكس عادي**: Linux **x86_64**
- 📱 **موبايل أندرويد (Termux)**: نسخة **ARM64** رسمية — المثبّت بيكتشفها تلقائي جوه proot-distro · **الدليل الكامل: [TERMUX.md](TERMUX.md)**

### 🔄 التحديث | Update

```bash
# من أي إصدار — أعد تشغيل المثبّت (بيحدّث الباينري ويسيّب الكونفيج والجسر زي ما هما)
curl -fsSL https://raw.githubusercontent.com/BlackXV2vip/exos/main/install | bash
...
./exos-agent serve --port 8799    # 🌐 web UI → http://127.0.0.1:8799
```
