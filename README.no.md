...
brew install anomalyco/tap/exos-agent # macOS og Linux (anbefalt, alltid oppdatert)
brew install exos-agent              # macOS og Linux (offisiell brew-formel, oppdateres sjeldnere)
...
Exos Agent er også tilgjengelig som en desktop-app. Last ned direkte fra [releases-siden](https://github.com/anomalyco/exos-agent/releases) eller [exos-agent.ai/download](https://exos-agent.ai/download).
...
1. `$EXOS_AGENT_INSTALL_DIR` - Egendefinert installasjonsmappe
...
Exos Agent har to innebygde agents du kan bytte mellom med `Tab`-tasten.
...
Les mer om [agents](https://exos-agent.ai/docs/agents).

### Dokumentasjon

For mer info om hvordan du konfigurerer Exos Agent, [**se dokumentasjonen**](https://exos-agent.ai/docs).

### Bidra

Hvis du vil bidra til Exos Agent, les [contributing docs](./CONTRIBUTING.md) før du sender en pull request.

### Bygge på Exos Agent

Hvis du jobber med et prosjekt som er relatert til Exos Agent og bruker "exos-agent" som en del av navnet; for eksempel "exos-agent-dashboard" eller "exos-agent-mobile", legg inn en merknad i README som presiserer at det ikke er bygget av Exos Agent-teamet og ikke er tilknyttet oss på noen måte.

---

**Bli med i fellesskapet** [Discord](https://discord.gg/exos-agent) | [X.com](https://x.com/exos-agent)
