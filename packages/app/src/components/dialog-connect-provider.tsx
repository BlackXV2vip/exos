import type { ProviderAuthAuthorization, ProviderAuthMethod } from "@exos-agent-ai/sdk/v2/client"
import { Button } from "@exos-agent-ai/ui/button"
import { useDialog } from "@exos-agent-ai/ui/context/dialog"
import { Dialog } from "@exos-agent-ai/ui/dialog"
import { Icon } from "@exos-agent-ai/ui/icon"
import { IconButton } from "@exos-agent-ai/ui/icon-button"
import { List, type ListRef } from "@exos-agent-ai/ui/list"
import { ProviderIcon } from "@exos-agent-ai/ui/provider-icon"
import { Spinner } from "@exos-agent-ai/ui/spinner"
import { Tag } from "@exos-agent-ai/ui/tag"
import { TextField } from "@exos-agent-ai/ui/text-field"
import { ButtonV2 } from "@exos-agent-ai/ui/v2/button-v2"
import { DialogBody, DialogHeader, DialogTitle, DialogV2 } from "@exos-agent-ai/ui/v2/dialog-v2"
import { TextInputV2 } from "@exos-agent-ai/ui/v2/text-input-v2"
...
function ProviderPicker(props: {
  ...
  const note = (id: string) => {
    ...
    if (id === "exos-agent-go") return language.t("dialog.provider.exosAgentGo.tagline")
    return undefined
  }
    ...
    >
      {(i) => (
        <div class="px-1.25 w-full flex items-center gap-x-3">
          ...
          <span>{i.name}</span>
          <Show when={i.id === "exos-agent"}>
            <div class="text-14-regular text-text-weak">{language.t("dialog.provider.exos-agent.tagline")}</div>
          </Show>
...
function ProviderPickerV2(props: {
  ...
  const featured = ["exos-agent", "exos-agent-go", "anthropic", "openai", "google", "openrouter", "vercel"]
                  ...
                  <For each={group.items()}>
                    {(provider) => (
                      ...
                      >
                        ...
                        <Show when={provider.id === "exos-agent" || provider.id === "exos-agent-go"}>
                          <span class="min-w-0 truncate font-[440] text-v2-text-text-muted">
                            {language.t(
                              provider.id === "exos-agent"
                                ? "dialog.provider.exos-agent.tagline"
                                : "dialog.provider.exosAgentGo.tagline",
                            )}
                          </span>
  ...
  function ApiAuthView() {
    ...
    if (newLayout())
      return (
        <div class="flex flex-col gap-5 px-3 text-[13px] font-[440] leading-5 tracking-[-0.04px] text-v2-text-text-muted">
          <Show
            when={provider().id === "exos-agent"}
            fallback={language.t("provider.connect.apiKey.description", { provider: provider().name })}
          >
            <div class="flex flex-col gap-5">
              <div>{language.t("provider.connect.exosAgentZen.line1")}</div>
              <div>{language.t("provider.connect.exosAgentZen.line2")}</div>
              <div>
                {language.t("provider.connect.exosAgentZen.visit.prefix")}
                <Link
                  href="https://exos-agent.ai/zen"
                  class="text-v2-text-text-base focus-visible:rounded-xs focus-visible:outline-2 focus-visible:outline-v2-border-border-focus"
                >
                  {language.t("provider.connect.exosAgentZen.visit.link")}
                </Link>
                {language.t("provider.connect.exosAgentZen.visit.suffix")}
              </div>
            </div>
          </Show>
    ...
    return (
      <div class="flex flex-col gap-6">
        <Switch>
          <Match when={provider().id === "exos-agent"}>
            <div class="flex flex-col gap-4">
              <div class="text-14-regular text-text-base">{language.t("provider.connect.exosAgentZen.line1")}</div>
              <div class="text-14-regular text-text-base">{language.t("provider.connect.exosAgentZen.line2")}</div>
              <div class="text-14-regular text-text-base">
                {language.t("provider.connect.exosAgentZen.visit.prefix")}
                <Link href="https://exos-agent.ai/zen" tabIndex={-1}>
                  {language.t("provider.connect.exosAgentZen.visit.link")}
                </Link>
                {language.t("provider.connect.exosAgentZen.visit.suffix")}
              </div>
