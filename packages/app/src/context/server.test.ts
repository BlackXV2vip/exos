  ...
  test("moves an existing canonical web bucket into local scope", () => {
    expect(
      migrateCanonicalLocalServerState(
        {
          list: [],
          projects: { "https://exos-agent.example.com": [{ worktree: "/remote", expanded: true }] },
          lastProject: { "https://exos-agent.example.com": "/remote" },
        },
        ServerConnection.Key.make("https://exos-agent.example.com"),
      ),
    ).toEqual({
      list: [],
      ...
      migrateCanonicalLocalServerState(
        {
          projects: {
            ...
            "https://exos-agent.example.com": [
              { worktree: "/local", expanded: true },
              { worktree: "/remote", expanded: true },
            ],
          },
          lastProject: { local: "/local", "https://exos-agent.example.com": "/remote" },
        },
        ServerConnection.Key.make("https://exos-agent.example.com"),
      ),
    ).toEqual({
