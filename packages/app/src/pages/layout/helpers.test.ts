...
import { type Session } from "@exos-agent-ai/sdk/v2/client"
import {
  childSessionOnPath,
  closeHomeProject,
  displayName,
...
describe("layout deep links", () => {
  test("parses open-project deep links", () => {
    expect(parseDeepLink("exos-agent://open-project?directory=/tmp/demo")).toBe("/tmp/demo")
  })

  test("ignores non-project deep links", () => {
    expect(parseDeepLink("exos-agent://other?directory=/tmp/demo")).toBeUndefined()
    expect(parseDeepLink("https://example.com")).toBeUndefined()
  })

  test("ignores malformed deep links safely", () => {
    expect(() => parseDeepLink("exos-agent://open-project/%E0%A4%A%")).not.toThrow()
    expect(parseDeepLink("exos-agent://open-project/%E0%A4%A%")).toBeUndefined()
  })
  ...
  test("ignores open-project deep links without directory", () => {
    expect(parseDeepLink("exos-agent://open-project")).toBeUndefined()
    expect(parseDeepLink("exos-agent://open-project?directory=")).toBeUndefined()
  })

  test("collects only valid open-project directories", () => {
    const result = collectOpenProjectDeepLinks([
      "exos-agent://open-project?directory=/a",
      "exos-agent://other?directory=/b",
      "exos-agent://open-project?directory=/c",
    ])
    expect(result).toEqual(["/a", "/c"])
  })

  test("parses new-session deep links with optional prompt", () => {
    expect(parseNewSessionDeepLink("exos-agent://new-session?directory=/tmp/demo")).toEqual({ directory: "/tmp/demo" })
    expect(parseNewSessionDeepLink("exos-agent://new-session?directory=/tmp/demo&prompt=hello%20world")).toEqual({
      directory: "/tmp/demo",
      prompt: "hello world",
    })
  })
  ...
  test("ignores new-session deep links without directory", () => {
    expect(parseNewSessionDeepLink("exos-agent://new-session")).toBeUndefined()
    expect(parseNewSessionDeepLink("exos-agent://new-session?directory=")).toBeUndefined()
  })

  test("collects only valid new-session deep links", () => {
    const result = collectNewSessionDeepLinks([
      "exos-agent://new-session?directory=/a",
      "exos-agent://open-project?directory=/b",
      "exos-agent://new-session?directory=/c&prompt=ship%20it",
    ])
  ...
  test("drains global deep links once", () => {
    const target = {
      __EXOS_AGENT__: {
        deepLinks: ["exos-agent://open-project?directory=/a"],
      },
    } as unknown as Window & { __EXOS_AGENT__?: { deepLinks?: string[] } }

    expect(drainPendingDeepLinks(target)).toEqual(["exos-agent://open-project?directory=/a"])
