...
import type { SessionNotFoundError } from "@exos-agent-ai/sdk/v2/client"
...
describe("parseReadableConfigInvalidError", () => {
  test("formats issues with file path", () => {
    ...
    expect(result).toBe(
      ["Arquivo de config em exos-agent.config.ts invalido: settings.host: Required", "mode: Invalid"].join("\n"),
    )
  })
