...
describe("WSL server settings presentation", () => {
  ...
  test("offers install and update only when Exos Agent needs attention", () => {
    expect(wslExosAgentAction(undefined)).toBeUndefined()
    expect(
      wslExosAgentAction({
        distro: "Debian",
        resolvedPath: null,
    ...
    ).toBe("Install Exos Agent")
    expect(
      wslExosAgentAction({
        distro: "Debian",
        resolvedPath: "/usr/local/bin/exos-agent",
        version: "1.2.2",
        expectedVersion: "1.2.3",
        matchesDesktop: false,
        error: null,
      }),
    ).toBe("Update Exos Agent")
    expect(
      wslExosAgentAction({
        distro: "Debian",
        resolvedPath: "/usr/local/bin/exos-agent",
  ...
  test("does not report ready when Exos Agent is present but cannot run", () => {
    const model = addServerViewModel({
      state: {
        ...readyWslState,
        installed: [{ name: "Debian", version: 2, isDefault: true }],
        ...
        exosAgentChecks: {
          Debian: {
            distro: "Debian",
            resolvedPath: "/home/me/.exos-agent/bin/exos-agent",
            version: null,
            expectedVersion: "1.2.3",
            matchesDesktop: null,
            error: "exos-agent is installed but could not run",
          },
        },
      },
      view: "main",
    ...
    expect(model.distroStatuses.Debian).toEqual({
      label: { key: "wsl.onboarding.installExosAgent" },
      tone: "warning",
    })
    expect(model.primaryButton.action).toBe("install-exos-agent")
  })
