...
export type AddServerPrimaryButton = {
  ...
  label: AddServerText
  disabled: boolean
  action: "install-exos-agent" | "add" | null
  loading: boolean
  width: string | null
}
...
export function wslExosAgentAction(check?: WslExosAgentCheck) {
  if (!check) return
  if (!check.resolvedPath) return "Install Exos Agent"
  if (check.matchesDesktop === false) return "Update Exos Agent"
}
...
export function addServerViewModel(input: {
  ...
  const exosAgentCheck = selectedDistro ? (state?.exosAgentChecks[selectedDistro] ?? null) : null
...
function addServerDistroStatus(input: {
  ...
  const check = input.state?.exosAgentChecks[input.name]
  if (!check) {
    if (input.probingAddable || (job?.kind === "probe-addable" && job.distros.includes(input.name))) {
      return checkingStatus()
    }
  ...
  if (check.matchesDesktop === false) return { label: { key: "wsl.onboarding.updateExosAgent" }, tone: "warning" }
  if (!check.resolvedPath) return { label: { key: "wsl.onboarding.distroStatus.exosAgentMissing" }, tone: "warning" }
  if (check.error) return { label: { key: "wsl.onboarding.installExosAgent" }, tone: "warning" }
...
function addServerPrimaryButton(input: {
  state: WslServersState | undefined
  selectedDistro: string | null
  exosAgentCheck: WslExosAgentCheck | null
  adding: boolean
  probingAddable: boolean
  ...
  const probingExosAgent =
    probingSelected ||
    (ready &&
      (!input.exosAgentCheck ||
        (!!input.selectedDistro &&
          input.state?.job?.kind === "probe-addable" &&
          input.state.job.distros.includes(input.selectedDistro))))
  const installingExosAgent =
    input.state?.job?.kind === "install-exos-agent" && input.state.job.distro === input.selectedDistro
  if (!ready || probingExosAgent) {
    return {
      variant: "contrast",
  ...
  if (!addServerExosAgentReady(input.exosAgentCheck)) {
    const update = !!input.exosAgentCheck?.resolvedPath && input.exosAgentCheck.matchesDesktop === false
    return {
      variant: "neutral",
      label: installingExosAgent
        ? { key: "wsl.onboarding.updatingExosAgent" }
        : update
          ? { key: "wsl.onboarding.updateExosAgent" }
          : { key: "wsl.onboarding.installExosAgent" },
      disabled: !!input.state?.job || input.adding,
      action: "install-exos-agent",
      loading: installingExosAgent,
...
function addServerExosAgentReady(check: WslExosAgentCheck | null) {
...
function addServerSelectedDistroSettled(state: WslServersState | undefined, selectedDistro: string | null) {
  ...
  return !!state.exosAgentChecks[selectedDistro]
}
...
export function addableProbePlan(input: {
  ...
  const pending = ordered.flatMap((item) => {
    ...
    if (wslDistroReady(state, item.name) && !state.exosAgentChecks[item.name]) return [`exos-agent:${item.name}`]
    return []
  })
