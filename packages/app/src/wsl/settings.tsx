...
import { wslExosAgentAction, wslRuntimeRetryable } from "./settings-model"
    ...
    <Show when={api}>
      <For each={props.servers()}>
        {(item) => {
          ...
          const check = () => wsl.data?.exosAgentChecks[item.config.distro]
          const exosAgentAction = () => wslExosAgentAction(check())
          const busy = () => wsl.data?.job?.kind === "install-exos-agent" && wsl.data.job.distro === item.config.distro
          return (
            <div class="settings-v2-servers-row">
              ...
              <div class="settings-v2-servers-actions">
                ...
                <Show when={exosAgentAction()}>
                  {(label) => (
                    <ButtonV2
                      size="small"
                      disabled={busy() || request.isPending}
                      onClick={() => api && request.mutate(() => api.installExosAgent(item.config.distro))}
                    >
