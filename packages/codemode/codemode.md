...
This is the living design and status document for `@exos-agent-ai/codemode` and its existing V2 Exos Agent adapter.
...
### V2 Exos Agent adapter

This section describes the `v2` branch integration. On `dev`, CodeMode is integrated through
`packages/exos-agent/src/tool/code-mode.ts`, where nested MCP calls run the `tool.execute.before` and
