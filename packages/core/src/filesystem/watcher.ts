...
import { FileSystemWatcher } from "@exos-agent-ai/schema/filesystem-watcher"
...
declare const EXOS_AGENT_LIBC: string | undefined
...
const watcher = lazy((): typeof import("@parcel/watcher") | undefined => {
  try {
    const libc = typeof EXOS_AGENT_LIBC === "undefined" ? undefined : EXOS_AGENT_LIBC
...
export class Service extends Context.Service<Service, Interface>()("@exos-agent/v2/FileWatcher") {}

const layer = Layer.effect(
  Service,
  Effect.gen(function* () {
    if (yield* Flag.EXOS_AGENT_EXPERIMENTAL_DISABLE_FILEWATCHER) return Service.of({})
    ...
    if (location.vcs && (yield* Flag.EXOS_AGENT_EXPERIMENTAL_FILEWATCHER)) {
