...
export function fileLogger(file = path.join(Global.Path.log, "exos-agent.log"), id: string = runID) {
...
export function minimumLogLevel() {
  const value = process.env.EXOS_AGENT_LOG_LEVEL?.toUpperCase()
  const levels = {
    DEBUG: "Debug",
    INFO: "Info",
    WARN: "Warn",
...
export function loggers() {
  return process.env.EXOS_AGENT_PRINT_LOGS === "1" ? [fileLogger(), stderrLogger] : [fileLogger()]
}
