...
import type { IntegrationOAuthMethodRegistration } from "@exos-agent-ai/plugin/v2/effect/integration"
import { define } from "@exos-agent-ai/plugin/v2/effect/plugin"
import type { CredentialValue } from "@exos-agent-ai/sdk/v2/types"
...
const defaultServer = "https://console.exos-agent.ai"
const clientID = "exos-agent-cli"
...
function oauth(http: HttpClient.HttpClient) {
  return {
    integrationID: Integration.ID.make("exos-agent"),
    method: {
      id: methodID,
      type: "oauth",
      label: "Exos Agent Console account",
    },
    authorize: () =>
      Effect.gen(function* () {
...
export const ExosAgentPlugin = define<HttpClient.HttpClient | EventV2.Service | Scope.Scope>({
  id: "exos-agent",
  effect: Effect.fn(function* (ctx) {
    ...
    const load = Effect.fn("ExosAgentPlugin.load")(function* () {
      const connection = yield* ctx.integration.connection.active("exos-agent")
      ...
      providers = credential
        ? yield* fetchProviders(http, credential).pipe(
            Effect.catch((cause) =>
              Effect.logWarning("failed to load Exos Agent provider config", { cause }).pipe(Effect.as(undefined)),
            ),
          )
        : undefined
    })
    ...
    yield* ctx.integration.transform((draft) => {
      draft.update("exos-agent", (integration) => {
        integration.name = "Exos Agent"
      })
      draft.method.update(oauth(http))
      draft.method.update({ integrationID: "exos-agent", method: { type: "key", label: "API key (service account)" } })
    })

    connected = (yield* ctx.integration.connection.active("exos-agent")) !== undefined
    yield* ctx.catalog.transform((catalog) => {
      for (const [providerID, item] of Object.entries(providers ?? {})) {
        catalog.provider.update(providerID, (provider) => {
          provider.integrationID = Integration.ID.make("exos-agent")
    ...
    yield* events.subscribe(Integration.Event.ConnectionUpdated).pipe(
      Stream.filter((event) => event.data.integrationID === Integration.ID.make("exos-agent")),
