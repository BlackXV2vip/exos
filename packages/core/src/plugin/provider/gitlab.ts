  ...
  effect: Effect.fn(function* (ctx) {
    yield* ctx.aisdk.sdk(
      Effect.fn(function* (evt) {
        if (evt.package !== "gitlab-ai-provider" && evt.package !== "@exos-agent/vendor-gitlab-ai-provider") return
        const mod = yield* Effect.promise(() => import("@exos-agent/vendor-gitlab-ai-provider"))
        evt.sdk = mod.createGitLab({
          ...
          aiGatewayHeaders: {
            "User-Agent": `exos-agent/${InstallationVersion} gitlab-ai-provider/${mod.VERSION} (${os.platform()} ${os.release()}; ${os.arch()})`,
    ...
    yield* ctx.aisdk.language(
      Effect.fn(function* (evt) {
        ...
        if (evt.model.api.id.startsWith("duo-workflow-")) {
          const gitlab = yield* Effect.promise(() => import("@exos-agent/vendor-gitlab-ai-provider")).pipe(Effect.orDie)
          const workflowRef =
