...
async function tmpdir() {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "exos-agent-core-test-"))
  return {
    path: dir,
  ...
  describe("Windows-specific", () => {
    fx.effect(
      ...
      Effect.gen(function* () {
        ...
        const out = yield* ChildProcessSpawner.ChildProcessSpawner.use((svc) =>
          svc.string(
            ChildProcess.make("set", ["EXOS_AGENT_TEST_SHELL"], {
              shell: true,
              extendEnv: true,
              env: { EXOS_AGENT_TEST_SHELL: "ok" },
            }),
          ),
        )
        expect(out).toContain("EXOS_AGENT_TEST_SHELL=ok")
      }),
    )
