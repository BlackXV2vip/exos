...
describe("Npm.sanitize", () => {
  test("keeps normal scoped package specs unchanged", () => {
    expect(Npm.sanitize("@exos-agent/acme")).toBe("@exos-agent/acme")
    expect(Npm.sanitize("@exos-agent/acme@1.0.0")).toBe("@exos-agent/acme@1.0.0")
    expect(Npm.sanitize("prettier")).toBe("prettier")
  })

  test("handles git https specs", () => {
    const spec = "acme@git+https://github.com/exos-agent/acme.git"
    const expected = win ? "acme@git+https_//github.com/exos-agent/acme.git" : spec
