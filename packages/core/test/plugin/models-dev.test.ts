...
import { ModelsDevPlugin } from "@exos-agent-ai/core/plugin/models-dev"
...
describe("ModelsDevPlugin", () => {
  ...
  it.effect("registers key methods for providers with environment variables", () =>
    Effect.acquireUseRelease(
      Effect.sync(() => {
        const previous = {
          path: Flag.EXOS_AGENT_MODELS_PATH,
          disabled: Flag.EXOS_AGENT_DISABLE_MODELS_FETCH,
        }
        Flag.EXOS_AGENT_MODELS_PATH = path.join(import.meta.dir, "fixtures", "models-dev.json")
      ...
      (previous) =>
        Effect.sync(() => {
          Flag.EXOS_AGENT_MODELS_PATH = previous.path
          Flag.EXOS_AGENT_DISABLE_MODELS_FETCH = previous.disabled
        }),
    ),
  )
})
