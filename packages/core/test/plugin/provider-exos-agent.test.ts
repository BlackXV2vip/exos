...
import { PluginHost } from "@exos-agent-ai/core/plugin/host"
import { ExosAgentPlugin } from "@exos-agent-ai/core/plugin/provider/exos-agent"
...
const addPlugin = Effect.fn(function* () {
  ...
  yield* ExosAgentPlugin.effect(host).pipe(
...
describe("ExosAgentPlugin", () => {
  it.effect("registers account and service account methods", () =>
    Effect.gen(function* () {
      yield* addPlugin()
      expect((yield* (yield* Integration.Service).get(Integration.ID.make("exos-agent")))?.methods).toEqual([
        {
  ...
  it.live("loads providers and models from the connected Exos Agent server", () =>
        ...
        Effect.gen(function* () {
          ...
          const provider = required(
            yield* eventually(
              ...
              (item) => item?.integrationID === Integration.ID.make("exos-agent"),
            ),
          )
          expect(provider).toMatchObject({
            name: "Remote",
            integrationID: "exos-agent",
            api: {
              type: "aisdk",
  ...
  it.effect("uses a public key and disables paid models without credentials", () =>
    withEnv({ EXOS_AGENT_API_KEY: undefined }, () =>
      Effect.gen(function* () {
        const catalog = yield* Catalog.Service
  ...
  it.effect("uses EXOS_AGENT_API_KEY as credentials", () =>
    withEnv({ EXOS_AGENT_API_KEY: "secret" }, () =>
      Effect.gen(function* () {
        const catalog = yield* Catalog.Service
        yield* catalog.transform((catalog) => {
          const provider = ProviderV2.Info.make({
  ...
  it.effect("uses configured provider env vars as credentials", () =>
    withEnv({ EXOS_AGENT_API_KEY: undefined, CUSTOM_EXOS_AGENT_API_KEY: "secret" }, () =>
      Effect.gen(function* () {
        const catalog = yield* Catalog.Service
        ...
        yield* integrations.transform((editor) => {
          editor.method.update({
            integrationID: Integration.ID.make("exos-agent"),
            method: { type: "env", names: ["CUSTOM_EXOS_AGENT_API_KEY"] },
          })
        })
        yield* catalog.transform((catalog) => {
          const provider = ProviderV2.Info.make({
  ...
  it.effect("ignores non-exos-agent providers and models", () =>
    withEnv({ EXOS_AGENT_API_KEY: undefined }, () =>
      Effect.gen(function* () {
        const catalog = yield* Catalog.Service
        yield* catalog.transform((catalog) => {
