...
- Running `bun dev` from `packages/exos-agent` starts the live interactive TUI. Do not run it as a blocking foreground command when you need to inspect the result.
- Start it in `tmux` instead: `tmux new-session -d -s exos-agent-dev 'bun dev'`.
- Capture the current TUI output with: `tmux capture-pane -pt exos-agent-dev`.
- Stop the session explicitly when done: `tmux kill-session -t exos-agent-dev`.
...
export class Service extends Context.Service<Service, Interface>()("@exos-agent/Foo") {}
