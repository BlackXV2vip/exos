...
Practical reference for a future `packages/server` split after the exos-agent
server moved to the Effect HttpApi backend.

## Current State

- The server still lives in `packages/exos-agent`.
...
Until enough shared service code lives outside `packages/exos-agent`, a future
`packages/server` should either:

- own pure HttpApi contracts only, or
- accept host-provided services/layers/callbacks from `packages/exos-agent`

It should not import `packages/exos-agent` services while `packages/exos-agent`
...
2. Move stable domain schemas into shared packages only when they no longer
   depend on exos-agent-local runtime modules.
3. Extract pure HttpApi contract modules into `packages/server` once the contract
   can compile without importing `packages/exos-agent` implementation details.
