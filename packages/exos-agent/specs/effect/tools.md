...
Practical reference for the current tool-migration state in `packages/exos-agent`.
...
Tool tests should use the existing Effect helpers in `packages/exos-agent/test/lib/effect.ts`:
