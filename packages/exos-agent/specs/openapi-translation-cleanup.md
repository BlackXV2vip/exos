...
Trim `packages/exos-agent/src/server/routes/instance/httpapi/public.ts` until OpenAPI generation is mostly a direct projection of the `HttpApi` route declarations, without breaking the generated SDK surface.
...
- `[x]` Add or extend `packages/exos-agent/test/server/httpapi-query-schema-drift.test.ts`.
...
- `[x]` `bun test --timeout 5000 test/server/httpapi-query-schema-drift.test.ts` from `packages/exos-agent`.
- `[x]` `bun typecheck` from `packages/exos-agent`.

### PR 2: Delete Spec-Only Workspace Query Injection

- `[x]` Edit `packages/exos-agent/src/server/routes/instance/httpapi/public.ts`.
...
- `[x]` `bun test --timeout 5000 test/server/httpapi-query-schema-drift.test.ts` from `packages/exos-agent`.
- `[x]` `bun dev generate > /tmp/exos-agent-openapi.json` from `packages/exos-agent`.
...
- Edit `packages/exos-agent/src/server/routes/instance/httpapi/public.ts`.
...
- `bun dev generate > /tmp/exos-agent-openapi.json` from `packages/exos-agent`.
...
- Check source schemas in files like `packages/exos-agent/src/session/schema.ts`, `packages/exos-agent/src/permission/schema.ts`, and pty schema definitions.
...
- Edit route group files under `packages/exos-agent/src/server/routes/instance/httpapi/groups/`.
- Replace SDK-visible `HttpApiError.BadRequest` / `HttpApiError.NotFound` with explicit error schemas from `packages/exos-agent/src/server/routes/instance/httpapi/errors.ts` or add new ones there.
