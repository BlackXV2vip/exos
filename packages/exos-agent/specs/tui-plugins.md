...
- Author package entrypoint is `@exos-agent-ai/plugin/tui`.
...
```json
{
  "$schema": "https://exos-agent.ai/tui.json",
  "theme": "smoke-theme",
  "leader_timeout": 2000,
  "keybinds": {
    "leader": "ctrl+x",
  ...
  "plugin": ["@acme/exos-agent-plugin@1.2.3", ["./plugins/demo.tsx", { "label": "demo" }]],
  "plugin_enabled": {
    "acme.demo": false
  },
  "attention": {
...
- Import types from `@exos-agent-ai/plugin/tui`.
- `@exos-agent-ai/plugin` exports `./tui` and declares optional peer deps on `@opentui/core` and `@opentui/solid`.

Minimal module shape:

```tsx
/** @jsxImportSource @opentui/solid */
import type { TuiPlugin, TuiPluginModule } from "@exos-agent-ai/plugin/tui"
...
- The value is a semver range checked against the running Exos Agent version.
- If the range is not satisfied, the plugin is skipped with a warning and a session error.
- If `engines.exos-agent` is absent, no check is performed (backward compatible).
...
- `exos-agent plugin <module>` and TUI install both run install → manifest read → config patch.
- Alias: `exos-agent plug <module>`.
- `-g` / `--global` writes into the global config dir.
- Local installs resolve target dir inside `patchPluginConfig`.
- For local scope, path is `<worktree>/.exos-agent` only when VCS is git and `worktree !== "/"`; otherwise `<directory>/.exos-agent`.
- Root-worktree fallback (`worktree === "/"` uses `<directory>/.exos-agent`) is covered by regression tests.
...
When `plugin` entries exist in a writable `.exos-agent` dir or `EXOS_AGENT_CONFIG_DIR`, Exos Agent installs `@exos-agent-ai/plugin` into that dir and writes:
...
That is what makes local config-scoped plugins able to import `@exos-agent-ai/plugin/tui`.
...
- The host already installs the default OpenTUI bundle (`default keys`, metadata fields, and enabled fields) plus Exos Agent's comma bindings, leader token, base layout fallback, pending-sequence helpers, and managed textarea layer.
...
Exos Agent registers a `mode` layer field on the host keymap. Plugins can use it to keep bindings active only in the relevant UI state.
...
Mode pushes are automatically tracked by the plugin runtime. If a plugin is disabled, fails during activation, or the TUI shuts down before the plugin calls the disposer, Exos Agent pops the plugin's pushed modes during plugin cleanup. Calling the disposer yourself is still recommended for component lifetimes; cleanup remains idempotent.
...
- For generic config-to-bindings helpers, import `createBindingLookup` from `@exos-agent-ai/plugin/tui`.
...
- `message` is required; `title` defaults to `"exos-agent"`; `notification` defaults to enabled with `when: "blurred"`; `sound` defaults to enabled with `when: "always"`.
...
- Config `attention.sounds` overrides active-pack sounds by slot. Failed loads fall back to the active pack and then `exos-agent.default`.
...
- Local plugins persist installed themes under the local `.exos-agent/themes` area near the plugin config source.
...
- `--pure` / `EXOS_AGENT_PURE` skips external TUI plugins only.
...
- Local smoke plugin: `.exos-agent/plugins/tui-smoke.tsx`
- Local vim plugin: `.exos-agent/plugins/tui-vim.tsx`
- Local smoke config: `.exos-agent/tui.json`
- Local smoke theme: `.exos-agent/plugins/smoke-theme.json`
