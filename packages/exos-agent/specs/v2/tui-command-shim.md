...
Delete `packages/exos-agent/src/cli/cmd/tui/plugin/command-shim.ts`.

In `packages/exos-agent/src/cli/cmd/tui/plugin/api.tsx`, remove:

- the `createCommandShim` import
- the `command: createCommandShim(...)` field from `createTuiApi(...)`

In `packages/exos-agent/src/cli/cmd/tui/plugin/runtime.ts`, remove:
...
- `bun typecheck` in `packages/exos-agent`
- TUI plugin loader tests in `packages/exos-agent` if runtime plugin API wiring changed
