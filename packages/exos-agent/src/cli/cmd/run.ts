import type { PermissionV1 } from "@exos-agent-ai/core/v1/permission"
import { FSUtil } from "@exos-agent-ai/core/fs-util"
// CLI entry point for `exos-agent run` and `exos-agent --mini`.
//
// Handles three modes:
//   1. Non-interactive (default): sends a single prompt, streams events to
//      stdout, and exits when the session goes idle.
//   2. Interactive local (`exos-agent --mini`): boots the split-footer direct mode
//      with an in-process server (no external HTTP).
//   3. Interactive attach (`exos-agent --mini --attach`): connects to a running
//      exos-agent server and runs interactive mode against it.
//
...
import { createExosAgentClient, type ExosAgentClient, type ToolPart } from "@exos-agent-ai/sdk/v2"
import { FormatError, FormatUnknownError } from "../error"
import { INTERACTIVE_INPUT_ERROR, resolveInteractiveStdin } from "./run/runtime.stdin"

type ModelInput = Parameters<ExosAgentClient["session"]["prompt"]>[0]["model"]
...
export const RunCommand = effectCmd({
  command: "run [message..]",
  describe: "run exos-agent with a message",
  ...
  builder: (yargs: Argv) =>
    yargs
      ...
      .option("attach", {
        type: "string",
        describe: "attach to a running exos-agent server (e.g., http://localhost:4096)",
      })
      .option("password", {
        alias: ["p"],
        type: "string",
        describe: "basic auth password (defaults to EXOS_AGENT_SERVER_PASSWORD)",
      })
      .option("username", {
        alias: ["u"],
        type: "string",
        describe: "basic auth username (defaults to EXOS_AGENT_SERVER_USERNAME or 'exos-agent')",
      })
      .option("dir", {
        type: "string",
  ...
  handler: Effect.fn("Cli.run")(function* (args) {
    ...
    yield* Effect.promise(async () => {
      ...
      async function session(sdk: ExosAgentClient): Promise<SessionInfo | undefined> {
        if (args.session) {
      ...
      async function share(sdk: ExosAgentClient, sessionID: string) {
      ...
      async function current(sdk: ExosAgentClient): Promise<string> {
        if (!args.attach) {
      ...
      async function attachAgent(sdk: ExosAgentClient) {
        if (!args.agent) return undefined
        const name = args.agent
      ...
      async function pickAgent(sdk: ExosAgentClient) {
        if (!args.agent) return undefined
        if (args.attach) {
      ...
      async function execute(sdk: ExosAgentClient) {
        ...
        async function loop(client: ExosAgentClient, events: Awaited<ReturnType<typeof sdk.event.subscribe>>) {
      ...
      const sdk = createExosAgentClient({
        baseUrl: "http://exos-agent.internal",
        fetch: fetchFn,
        directory,
      })
      await execute(sdk)
