...
//
// Enable with EXOS_AGENT_DIRECT_TRACE=1. Writes one JSON line per event to
// ~/.local/share/exos-agent/log/direct/<timestamp>-<pid>.jsonl. Also writes
...
export function trace(): Trace | undefined {
  ...
  if (!process.env.EXOS_AGENT_DIRECT_TRACE) {
    state = false
    return undefined
  }
