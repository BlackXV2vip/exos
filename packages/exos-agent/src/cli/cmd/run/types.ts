// Shared type vocabulary for the direct interactive mode (`exos-agent --mini`).
//
...
import type { ExosAgentClient, PermissionRequest, QuestionRequest, ToolPart } from "@exos-agent-ai/sdk/v2"
import type { TuiConfig } from "@exos-agent-ai/tui/config"
...
type PromptModel = Parameters<ExosAgentClient["session"]["prompt"]>[0]["model"]
type PromptInput = Parameters<ExosAgentClient["session"]["prompt"]>[0]

export type RunPromptPart = NonNullable<PromptInput["parts"]>[number]

export type RunCommand = NonNullable<Awaited<ReturnType<ExosAgentClient["command"]["list"]>>["data"]>[number]

export type RunProvider = NonNullable<Awaited<ReturnType<ExosAgentClient["provider"]["list"]>>["data"]>["all"][number]
...
export type RunAgent = NonNullable<Awaited<ReturnType<ExosAgentClient["app"]["agents"]>>["data"]>[number]

type RunResourceMap = NonNullable<Awaited<ReturnType<ExosAgentClient["experimental"]["resource"]["list"]>>["data"]>
...
export type PermissionReply = Parameters<ExosAgentClient["permission"]["reply"]>[0]

export type QuestionReply = Parameters<ExosAgentClient["question"]["reply"]>[0]

export type QuestionReject = Parameters<ExosAgentClient["question"]["reject"]>[0]
