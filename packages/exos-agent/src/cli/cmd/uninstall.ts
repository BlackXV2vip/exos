...
export const UninstallCommand = {
  command: "uninstall",
  describe: "uninstall exos-agent and remove all related files",
  builder: (yargs: Argv) =>
    yargs
      .option("keep-config", {
        alias: "c",
  ...
  handler: async (args: UninstallArgs) => {
    ...
    UI.println(UI.logo("  "))
    UI.empty()
    prompts.intro("Uninstall Exos Agent")
...
async function showRemovalSummary(targets: RemovalTargets, method: Installation.Method) {
  ...
  if (method !== "curl" && method !== "unknown") {
    const cmds: Record<string, string> = {
      npm: "npm uninstall -g exos-agent-ai",
      pnpm: "pnpm uninstall -g exos-agent-ai",
      bun: "bun remove -g exos-agent-ai",
      yarn: "yarn global remove exos-agent-ai",
      brew: "brew uninstall exos-agent",
      choco: "choco uninstall exos-agent",
      scoop: "scoop uninstall exos-agent",
    }
...
async function executeUninstall(method: Installation.Method, targets: RemovalTargets) {
  ...
  if (method !== "curl" && method !== "unknown") {
    ...
    if (cmd) {
      ...
      const result = await Process.run(method === "choco" ? ["choco", "uninstall", "exos-agent", "-y", "-r"] : cmd, {
        nothrow: true,
      })
      if (result.code !== 0) {
  ...
  UI.empty()
  prompts.log.success("Thank you for using Exos Agent!")
}
...
async function getShellConfigFile(): Promise<string | null> {
  ...
  for (const file of candidates) {
    ...
    if (content.includes("# exos-agent") || content.includes(".exos-agent/bin")) {
      return file
    }
  }
...
async function cleanShellConfig(file: string) {
  ...
  for (const line of lines) {
    ...
    if (skip) {
      skip = false
      if (trimmed.includes(".exos-agent/bin") || trimmed.includes("fish_add_path")) {
        continue
      }
    }

    if (
      (trimmed.startsWith("export PATH=") && trimmed.includes(".exos-agent/bin")) ||
      (trimmed.startsWith("fish_add_path") && trimmed.includes(".exos-agent"))
    ) {
      continue
    }
