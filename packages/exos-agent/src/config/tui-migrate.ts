...
const TUI_SCHEMA_URL = "https://exos-agent.ai/tui.json"
...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from exos-agent.json files
...
async function exosAgentFiles(input: { directories: string[]; cwd: string }) {
  const files = [
    ...ConfigPaths.fileInDirectory(Global.Path.config, "exos-agent"),
    ...(await Filesystem.findUp(["exos-agent.json", "exos-agent.jsonc"], input.cwd, undefined, { rootFirst: true })),
  ]
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "exos-agent"))
  }
  if (Flag.EXOS_AGENT_CONFIG) files.push(Flag.EXOS_AGENT_CONFIG)
