...
import { isRecord } from "@exos-agent-ai/tui/util/record"
...
import { TuiKeybind } from "@exos-agent-ai/tui/config/keybind"
...
import { TuiConfig } from "@exos-agent-ai/tui/config"
...
export class Service extends Context.Service<Service, Interface>()("@exos-agent/TuiConfig") {}
...
const loadState = Effect.fn("TuiConfig.loadState")(function* (ctx: { directory: string }) {
  ...
  const load = (text: string, configFilepath: string): Effect.Effect<Info> =>
    Effect.gen(function* () {
      ...
      // (mirroring the old exos-agent.json shape) still get their settings applied.
  ...
  // Every config dir we may read from: global config dir, any `.exos-agent`
  // folders between cwd and home, and EXOS_AGENT_CONFIG_DIR.
  const directories = yield* ConfigPaths.directories(ctx.directory)
  yield* Effect.promise(() => migrateTuiConfig({ directories, cwd: ctx.directory }))

  const projectFiles = Flag.EXOS_AGENT_DISABLE_PROJECT_CONFIG ? [] : yield* ConfigPaths.files("tui", ctx.directory)
  ...
  // 2. Explicit EXOS_AGENT_TUI_CONFIG override, if set.
  if (Flag.EXOS_AGENT_TUI_CONFIG) {
    const configFile = Flag.EXOS_AGENT_TUI_CONFIG
  ...
  // 4. `.exos-agent` directories (and EXOS_AGENT_CONFIG_DIR) discovered while
  // walking up the tree. Also returned below so callers can install plugin
  // dependencies from each location.
  const dirs = unique(directories).filter((dir) => dir.endsWith(".exos-agent") || dir === Flag.EXOS_AGENT_CONFIG_DIR)

  for (const dir of dirs) {
    if (!dir.endsWith(".exos-agent") && dir !== Flag.EXOS_AGENT_CONFIG_DIR) continue
