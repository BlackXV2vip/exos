...
import { EventSequenceTable, EventTable } from "@exos-agent-ai/core/event/sql"
...
import { WorkspaceEvent } from "@exos-agent-ai/schema/workspace-event"
...
export class Service extends Context.Service<Service, Interface>()("@exos-agent/Workspace") {}
...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const create = Effect.fn("Workspace.create")(function* (input: CreateInput) {
      ...
      const env = {
        EXOS_AGENT_AUTH_CONTENT: JSON.stringify(yield* auth.all()),
        EXOS_AGENT_WORKSPACE_ID: config.id,
        EXOS_AGENT_EXPERIMENTAL_WORKSPACES: "true",
