...
export type PatchDeps = {
  ...
  files: (dir: string, name: "exos-agent" | "tui") => string[]
}
...
function patchDir(input: PatchInput) {
  ...
  return path.join(root, ".exos-agent")
}

function patchName(kind: Kind): "exos-agent" | "tui" {
  if (kind === "server") return "exos-agent"
  return "tui"
}
