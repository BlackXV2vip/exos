...
export const DEPRECATED_PLUGIN_PACKAGES = ["exos-agent-openai-codex-auth", "exos-agent-copilot-auth"]
...
export async function checkPluginCompatibility(target: string, exosAgentVersion: string, pkg?: PluginPackage) {
  if (!semver.valid(exosAgentVersion) || semver.major(exosAgentVersion) === 0) return
  const hit = pkg ?? (await readPluginPackage(target).catch(() => undefined))
  if (!hit) return
  const engines = hit.json.engines
  if (!isRecord(engines)) return
  const range = engines["exos-agent"]
  if (typeof range !== "string") return
  if (!semver.satisfies(exosAgentVersion, range)) {
    throw new Error(`Plugin requires exos-agent ${range} but running ${exosAgentVersion}`)
  }
}
