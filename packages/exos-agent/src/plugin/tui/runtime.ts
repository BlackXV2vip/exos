...
import {
  ...
  type TuiSlotPlugin,
  type TuiTheme,
} from "@exos-agent-ai/plugin/tui"
import path from "path"
import { fileURLToPath } from "url"
import { TuiConfig } from "@/config/tui"
import { AppNodeBuilder } from "@exos-agent-ai/core/effect/app-node-builder"
import { errorData, errorMessage } from "@exos-agent-ai/tui/util/error"
...
import { hasTheme, upsertTheme } from "@exos-agent-ai/tui/context/theme"
import { Global } from "@exos-agent-ai/core/global"
import { Filesystem } from "@/util/filesystem"
import { Process } from "@/util/process"
import { Flock } from "@exos-agent-ai/core/util/flock"
import { Flag } from "@exos-agent-ai/core/flag/flag"
import { internalTuiPlugins, type InternalTuiPlugin } from "./internal"
import type { HostPluginApi, HostSlots } from "@exos-agent-ai/tui/plugin/slots"
import { ConfigPlugin } from "@/config/plugin"
import { ConfigPluginV1 } from "@exos-agent-ai/core/v1/config/plugin"
import { createCommandShim } from "@exos-agent-ai/tui/plugin/command-shim"
import { RuntimeFlags } from "@/effect/runtime-flags"
import { Effect } from "effect"
import { createPluginRuntime, type PluginRuntime, type TuiPluginHost } from "@exos-agent-ai/tui/plugin/runtime"
...
function createThemeInstaller(
  ...
  return async (file) => {
    ...
    const local_dir =
      path.basename(source_dir) === ".exos-agent"
        ? path.join(source_dir, "themes")
        : path.join(source_dir, ".exos-agent", "themes")
...
function defaultPluginOrigin(state: RuntimeState, spec: string): ConfigPlugin.Origin {
  return {
    spec,
    scope: "local",
    source: state.api.state.path.config || path.join(state.directory, ".exos-agent", "tui.json"),
  }
}
...
async function load(input: {
  ...
  try {
    ...
    const records = Flag.EXOS_AGENT_PURE ? [] : pluginOrigins
    if (Flag.EXOS_AGENT_PURE && pluginOrigins.length) {
    }
