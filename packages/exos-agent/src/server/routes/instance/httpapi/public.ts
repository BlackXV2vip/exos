...
import { ExosAgentHttpApi } from "./api"
...
const LegacyComponentDescriptions: Record<string, string> = {
  LogLevel: "Log level",
  ServerConfig: "Server configuration for exos-agent serve and web commands",
...
function fixSelfReferencingComponents(spec: OpenApiSpec) {
  ...
  const raw: OpenApiSpec = OpenApi.fromApi(ExosAgentHttpApi)
...
export const PublicApi = ExosAgentHttpApi.annotateMerge(
  OpenApi.annotations({
    title: "exos-agent",
    version: "1.0.0",
    description: "exos-agent api",
