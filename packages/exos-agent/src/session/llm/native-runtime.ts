...
import type { LLMClientShape } from "@exos-agent-ai/llm/route"
...
function statusWithFetch(
  ...
  if (providerID !== "openai" && providerID !== "anthropic" && !providerID.startsWith("exos-agent"))
    return { type: "unsupported", reason: "provider is not openai, exos-agent, or anthropic" }
...
export function stream(input: StreamInput): StreamResult {
  ...
  // Integration point with @exos-agent-ai/llm: native-request lowers session data
...
export function nativeTools(tools: Record<string, Tool>, input: Pick<StreamInput, "messages" | "abort">) {
  return Object.fromEntries(
    Object.entries(tools).map(([name, item]) => [
      name,
      // Tool execution remains exos-agent-owned. The native runtime only adapts
      // the @exos-agent-ai/llm tool call back into the AI SDK Tool.execute shape.
      NativeTool.make({
