...
import { path } from "@exos-agent-ai/core/effect/app-node-platform"
...
import type { ProjectV2 } from "@exos-agent-ai/core/project"
...
import { WorktreeEvent } from "@exos-agent-ai/schema/worktree-event"
...
export class Service extends Context.Service<Service, Interface>()("@exos-agent/Worktree") {}
  ...
  Effect.gen(function* () {
    ...
    const candidate = Effect.fn("Worktree.candidate")(function* (input: {
      ...
      for (const attempt of Array.from({ length: MAX_NAME_ATTEMPTS }, (_, i) => i)) {
        ...
        const branch = input.detached ? undefined : `exos-agent/${name}`
