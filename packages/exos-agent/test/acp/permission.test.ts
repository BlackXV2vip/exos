...
import type { Event, ExosAgentClient } from "@exos-agent-ai/sdk/v2"
...
type PermissionReplyParams = Parameters<ExosAgentClient["permission"]["reply"]>[0]
...
async function tempFile(name: string, content: string) {
  const dir = await mkdtemp(path.join(tmpdir(), "exos-agent-acp-permission-"))
