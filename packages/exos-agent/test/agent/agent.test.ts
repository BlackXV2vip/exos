...
it.instance("plan agent denies edits except .exos-agent/plans/*", () =>
  Effect.gen(function* () {
    ...
    expect(Permission.evaluate("edit", ".exos-agent/plans/foo.md", plan!.permission).action).toBe("allow")
  }),
)
...
it.instance(
  ...
  () =>
    Effect.gen(function* () {
      ...
      const skillDir = path.join(test.directory, ".exos-agent", "skill", "perm-skill")
...
`,
      ...
      const home = process.env.EXOS_AGENT_TEST_HOME
      process.env.EXOS_AGENT_TEST_HOME = test.directory
