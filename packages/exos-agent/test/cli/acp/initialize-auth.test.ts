...
describe("exos-agent acp initialize/auth subprocess", () => {
  cliIt.live(
    ...
    ({ exosAgent }) =>
      Effect.gen(function* () {
        ...
        expect(initialized.agentInfo?.name).toBe("Exos Agent")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ exosAgent }) =>
      Effect.gen(function* () {
        const acp = yield* createAcpClient({ exosAgent })
        const initialized = yield* initialize(acp)

        expect(initialized.authMethods?.[0]?.id).toBe("exos-agent-login")
        expect(initialized.authMethods?.[0]?._meta?.["terminal-auth"]).toBeDefined()
        expect(yield* acp.request<AuthenticateResponse>("authenticate", { methodId: "exos-agent-login" })).toMatchObject({
          result: {},
        })
        ...
        expect(JSON.stringify(rejected.error)).not.toContain(process.env.EXOS_AGENT_AUTH_CONTENT ?? "not-present")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ exosAgent }) =>
      Effect.gen(function* () {
        ...
        expect(initialized.result?.authMethods?.[0]?.id).toBe("exos-agent-login")
