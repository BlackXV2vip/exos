...
// terminal sizes. The default exos-agent tui command is excluded —
// `exos-agent --help` includes an ASCII banner that pulls in the install
...
// Top-level commands. Order matches what `exos-agent --help` prints today;
...
// top-level help on `--help` and exits 1; not a real exos-agent command.
const TOP_LEVEL = [
  "acp",
  "mcp",
  "attach",
...
describe("exos-agent CLI help-text snapshots", () => {
    ...
    ({ exosAgent }) =>
      Effect.gen(function* () {
        ...
        for (const { argv, result } of results) {
          ...
          expect(normalize(result.stderr)).toMatchSnapshot(`exos-agent ${argv.join(" ")} --help`)
        }
