...
import { ExosAgentClient, type GlobalEvent } from "@exos-agent-ai/sdk/v2"
import { createSessionTransport } from "@/cli/cmd/run/stream.transport"
import type { FooterApi, FooterEvent, LocalReplayRow, RunFilePart, StreamCommit } from "@/cli/cmd/run/types"

type EventStream = Awaited<ReturnType<ExosAgentClient["event"]["subscribe"]>>["stream"]
type GlobalEventStream = Awaited<ReturnType<ExosAgentClient["global"]["event"]>>["stream"]
type SdkEvent = EventStream extends AsyncGenerator<infer T, unknown, unknown> ? T : never
type SessionMessage = NonNullable<Awaited<ReturnType<ExosAgentClient["session"]["messages"]>>["data"]>[number]
type SessionChild = NonNullable<Awaited<ReturnType<ExosAgentClient["session"]["children"]>>["data"]>[number]
type SessionToolPart = Extract<SessionMessage["parts"][number], { type: "tool" }>
type SessionStatusMap = NonNullable<Awaited<ReturnType<ExosAgentClient["session"]["status"]>>["data"]>
...
function ok<T>(data: T) {
  return Promise.resolve({
    data,
    error: undefined,
    request: new Request("https://exos-agent.test"),
...
function sdk(
  input: {
    ...
    subscribe?: ExosAgentClient["event"]["subscribe"]
    globalEvent?: ExosAgentClient["global"]["event"]
    promptAsync?: ExosAgentClient["session"]["promptAsync"]
    status?: ExosAgentClient["session"]["status"]
    messages?: ExosAgentClient["session"]["messages"]
    children?: ExosAgentClient["session"]["children"]
    permissions?: ExosAgentClient["permission"]["list"]
    questions?: ExosAgentClient["question"]["list"]
  } = {},
) {
  const client = new ExosAgentClient()

  const subscribe: ExosAgentClient["event"]["subscribe"] = input.subscribe ?? (() => sse(input.stream ?? emptyStream()))
  const globalEvent: ExosAgentClient["global"]["event"] =
    input.globalEvent ?? (() => globalSse(input.globalStream ?? wrapGlobalStream(input.stream ?? emptyStream())))
  const promptAsync: ExosAgentClient["session"]["promptAsync"] = input.promptAsync ?? (() => ok(undefined))
  const status: ExosAgentClient["session"]["status"] = input.status ?? (() => ok({}))
  const messages: ExosAgentClient["session"]["messages"] = input.messages ?? (() => ok([]))
  const children: ExosAgentClient["session"]["children"] = input.children ?? (() => ok([]))
  const permissions: ExosAgentClient["permission"]["list"] = input.permissions ?? (() => ok([]))
  const questions: ExosAgentClient["question"]["list"] = input.questions ?? (() => ok([]))
