...
import type { HostPluginApi } from "@exos-agent-ai/tui/plugin/slots"
...
export function createTuiPluginApi(opts: Opts = {}): HostPluginApi {
  ...
  const own = createExosAgentClient({
  ...
  let selected = opts.theme?.selected ?? "exos-agent"
  const set =
    opts.theme?.set ??
    ((name: string) => {
  ...
  return {
    ...
    attention: {
      ...
      soundboard: {
        ...
        current: () => opts.attention?.soundboard?.current?.() ?? "exos-agent.default",
