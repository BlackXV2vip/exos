// Subprocess test harness for the exos-agent CLI. Spawns the real binary against
...
//
// Configuration flows through exos-agent's built-in test affordances:
//   - EXOS_AGENT_CONFIG_CONTENT      : provider config inline, no files to find
//   - EXOS_AGENT_TEST_HOME           : pins os.homedir() → tmpdir
//   - EXOS_AGENT_DISABLE_PROJECT_CONFIG : skip walking up for exosAgent.json
//   - EXOS_AGENT_PURE                : skip external plugin discovery + install
//   - EXOS_AGENT_DISABLE_AUTOUPDATE / AUTOCOMPACT / MODELS_FETCH : no background work
// Plus HOME / XDG_* pointing at the tmpdir for belt-and-suspenders isolation.
//
// Today only `exosAgent.run` is fully wired. The shape supports adding more
// builders (`exosAgent.serve(opts)`, `exosAgent.acp(opts)`, `exosAgent.auth(...)`)
// without changing the fixture. Long-lived commands like `serve` will need a
// different return shape — see the TODO at the bottom of ExosAgentCli.
...
import { AppProcess } from "@exos-agent-ai/core/process"
...
const exosAgentRoot = path.resolve(import.meta.dir, "../../")
const cliEntry = path.join(exosAgentRoot, "src/index.ts")
...
function isolatedEnv(home: string, configJson: string): Record<string, string> {
  return {
    EXOS_AGENT_TEST_HOME: home,
    HOME: home,
    XDG_CONFIG_HOME: path.join(home, ".config"),
    XDG_DATA_HOME: path.join(home, ".local/share"),
    XDG_STATE_HOME: path.join(home, ".local/state"),
    XDG_CACHE_HOME: path.join(home, ".cache"),
    EXOS_AGENT_CONFIG_CONTENT: configJson,
    EXOS_AGENT_DISABLE_PROJECT_CONFIG: "1",
    EXOS_AGENT_PURE: "1",
    EXOS_AGENT_DISABLE_AUTOUPDATE: "1",
    EXOS_AGENT_DISABLE_AUTOCOMPACT: "1",
    EXOS_AGENT_DISABLE_MODELS_FETCH: "1",
    EXOS_AGENT_AUTH_CONTENT: "{}",
  }
}
...
// Typed equivalent of constructing argv for `exos-agent run`. New flags should
...
// `exos-agent serve` is a long-lived process — it never exits on its own.
...
// `exos-agent acp` speaks newline-delimited JSON-RPC over stdin/stdout. It is
...
export type ExosAgentCli = {
  ...
  // Spawn `exos-agent serve` and wait until it's listening. Long-lived: the
  ...
  // Spawn `exos-agent acp` and return a duplex JSON-RPC handle. Long-lived:
...
export function withCliFixture<A, E>(
  ...
  return Effect.gen(function* () {
    ...
    const spawn = Effect.fn("exosAgent.spawn")(function* (args: string[], opts?: SpawnOpts) {
    ...
    const runOpts = (opts?: RunOpts): SpawnOpts | undefined => {
      ...
      return {
        ...
        env: {
          ...opts.env,
          EXOS_AGENT_CONFIG_CONTENT: JSON.stringify({
    ...
    const startRun = Effect.fn("exosAgent.startRun")(function* (message: string, opts?: RunOpts) {
      const start = Date.now()
    ...
    const serve = Effect.fn("exosAgent.serve")(function* (opts?: ServeOpts) {
      const argv = ["serve"]
      ...
      //   "exos-agent server listening on http://<host>:<port>"
      ...
      const match = yield* Deferred.await(readyDeferred).pipe(
        Effect.timeoutOrElse({
          ...
          orElse: () =>
            Effect.fail(
              new Error(
                `exos-agent serve did not become ready within ${readyTimeoutMs}ms\n` +
    ...
    const acp = Effect.fn("exosAgent.acp")(function* (opts?: AcpOpts) {
      const argv = ["acp"]
    ...
    // and hit endpoints on `exosAgent.serve()` without rolling their own fetch.
  }).pipe(
    Effect.provide(
      Layer.mergeAll(
        TestLLMServer.layer,
...
function expectExit(result: RunResult, expected: number, label = "exos-agent") {
...
// every call site. Use this for any test that needs the exos-agent CLI fixture.
//
...
// resources (e.g. `exosAgent.serve`) without an extra `Effect.scoped` wrapper —
