...
function requestServer(path: string, directory: string, init: RequestInit = {}) {
  ...
  headers.set("x-exos-agent-directory", directory)
      ...
      try {
        const response = yield* requestDefault(url.toString(), dir, {
          ...
          headers: {
            "accept-encoding": "br",
            "content-type": "application/json",
            "x-exos-agent-workspace": "internal",
          },
          body: JSON.stringify({ $schema: "https://exos-agent.ai/config.json" }),
        })
        ...
        expect(forwarded[0]?.headers).not.toHaveProperty("x-exos-agent-directory")
        expect(forwarded[0]?.headers).not.toHaveProperty("x-exos-agent-workspace")
