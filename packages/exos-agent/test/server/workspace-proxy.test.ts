...
describe("HttpApi workspace proxy", () => {
  ...
  it.live("strips exos-agent-internal headers and merges extra headers", () =>
    Effect.gen(function* () {
      ...
      expect(forwarded["x-exos-agent-directory"]).toBeUndefined()
      expect(forwarded["x-exos-agent-workspace"]).toBeUndefined()
