...
const stateLayer = Layer.effectDiscard(
  Effect.gen(function* () {
    const original = {
      EXOS_AGENT_EXPERIMENTAL_WORKSPACES: Flag.EXOS_AGENT_EXPERIMENTAL_WORKSPACES,
    }
    ...
    yield* Effect.addFinalizer(() =>
      Effect.promise(async () => {
        Flag.EXOS_AGENT_EXPERIMENTAL_WORKSPACES = original.EXOS_AGENT_EXPERIMENTAL_WORKSPACES
        await resetDatabase()
      }),
    )
  }),
