...
import { SessionV1 } from "@exos-agent-ai/core/v1/session"
import { ModelsDev } from "@exos-agent-ai/core/models-dev"
import { HttpRecorder } from "@exos-agent-ai/http-recorder"
import { HttpRecorderInternal } from "@exos-agent-ai/http-recorder/internal"
...
import { LLMEvent, LLMResponse } from "@exos-agent-ai/llm"
import { RequestExecutor } from "@exos-agent-ai/llm/route"
...
const zenURL = (connection: string) => `https://console.exos-agent.ai/proxy/connections/${connection}/v1`
...
function decodeRecordOpenAIOAuth() {
  const value = process.env.EXOS_AGENT_RECORD_OPENAI_AUTH
...
const RECORDED_SCENARIOS = [
  {
    ...
    tags: ["exos-agent", "native", "tool-loop"],
    canRecord: () => Boolean(envValue("EXOS_AGENT_RECORD_OPENAI_API_KEY", "OPENAI_API_KEY")),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.openai,
        name: "OpenAI",
        ...
        model,
        options: {
          apiKey: envValue("EXOS_AGENT_RECORD_OPENAI_API_KEY", "OPENAI_API_KEY") ?? "fixture-openai-key",
  ...
  },
  {
    ...
    protocol: "openai-responses",
    tags: ["exos-agent", "native", "oauth", "tool-loop"],
  ...
  },
  {
    ...
    config: (model) =>
      providerConfig({
        ...
        api: zenURL(process.env.EXOS_AGENT_RECORD_ZEN_CONNECTION ?? "fixture"),
        model,
        options: {
          apiKey: process.env.EXOS_AGENT_RECORD_CONSOLE_TOKEN ?? "fixture-console-token",
          headers: { "x-org-id": process.env.EXOS_AGENT_RECORD_ZEN_ORG_ID ?? "fixture-org" },
        },
      }),
  },
  {
    ...
    tags: ["exos-agent", "native", "tool-loop"],
    canRecord: () => Boolean(envValue("EXOS_AGENT_RECORD_ANTHROPIC_API_KEY", "ANTHROPIC_API_KEY")),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.anthropic,
        name: "Anthropic",
        ...
        model,
        options: {
          apiKey: envValue("EXOS_AGENT_RECORD_ANTHROPIC_API_KEY", "ANTHROPIC_API_KEY") ?? "fixture-anthropic-key",
...
const selectedScenarios = new Set(
  (envValue("EXOS_AGENT_RECORDED_SCENARIO", "RECORDED_PROVIDER") ?? "")
    .split(",")
...
const recordError = (scenario: RecordedScenario) =>
  scenario.id === "openai-oauth"
    ? "Set EXOS_AGENT_RECORD_OPENAI_AUTH to an OAuth auth JSON object in the recording environment."
...
function recordedNativeLLMLayer(scenario: RecordedScenario) {
  ...
  // Only the HTTP client is recorded; RequestExecutor and the exos-agent LLM stack remain real.
  const metadata = {
...
const writeConfig = (directory: string, scenario: RecordedScenario, model: ModelsDev.Provider["models"][string]) =>
  Effect.promise(() =>
    Bun.write(
      path.join(directory, "exos-agent.json"),
      JSON.stringify({ $schema: "https://exos-agent.ai/config.json", ...scenario.config(model) }),
    ),
  )
