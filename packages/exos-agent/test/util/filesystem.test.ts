...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "exos-agent.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "exos-agent.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "exos-agent.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "exos-agent.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["exos-agent.json", "exos-agent.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })
      ...
      expect(result).toEqual([
        ...
        path.join(project, "exos-agent.json"),
        path.join(project, "exos-agent.jsonc"),
      ])
    })
  })
