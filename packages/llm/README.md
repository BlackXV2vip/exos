...
Schema-first LLM core for exos-agent. One typed request, response, event, and tool language; provider quirks live in adapters, not in calling code.

```ts
import { Effect } from "effect"
import { LLM, LLMClient } from "@exos-agent-ai/llm"
import { OpenAI } from "@exos-agent-ai/llm/providers"
...
```ts
import { OpenAI, CloudflareAIGateway } from "@exos-agent-ai/llm/providers"
