...
import { PromptInput } from "@exos-agent-ai/schema/prompt-input"
import { Session } from "@exos-agent-ai/schema/session"
import { Project } from "@exos-agent-ai/schema/project"
import { AbsolutePath, NonNegativeInt, PositiveInt, RelativePath, statics } from "@exos-agent-ai/schema/schema"
...
import { SessionEvent } from "@exos-agent-ai/schema/session-event"
    ...
    .add(
      HttpApiEndpoint.get("session.active", "/api/session/active", {
        ...
        OpenApi.annotations({
          ...
          description:
            "Retrieve foreground Session drains currently owned by this Exos Agent process. Sessions absent from the result are inactive.",
        }),
      ),
    )
    .add(
      ...
      HttpApiEndpoint.post("session.interrupt", "/api/session/:sessionID/interrupt", {
        ...
        .annotateMerge(
          OpenApi.annotations({
            ...
            description: "Interrupt active execution owned by this Exos Agent process. Idle interruption is a no-op.",
          }),
        ),
    )
    .add(
