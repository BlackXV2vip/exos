...
function ref(request: HttpServerRequest.HttpServerRequest): Location.Ref {
  ...
  const workspaceID = query.get("location[workspace]") || request.headers["x-exos-agent-workspace"]
  const directory =
    query.get("location[directory]") ||
    (request.headers["x-exos-agent-directory"] ? decode(request.headers["x-exos-agent-directory"]) : process.cwd())
