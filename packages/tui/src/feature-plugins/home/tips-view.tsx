import type { TuiPluginApi } from "@exos-agent-ai/plugin/tui"
...
export function Tips(props: { api: TuiPluginApi; connected?: boolean }) {
  ...
  const shortcuts: Shortcuts = {
    ...
    statusView: useCommandShortcut("exos-agent.status"),
...
const TIPS: Tip[] = [
  ...
  "Run {highlight}/share{/highlight} to create a public exos-agent.ai link",
  ...
  "Create {highlight}exos-agent.json{/highlight} for server settings, and {highlight}tui.json{/highlight} for TUI",
  "Place TUI settings in {highlight}~/.config/exos-agent/tui.json{/highlight} for global config",
  "Add {highlight}$schema{/highlight} to your config for autocomplete in your editor",
  "Configure {highlight}model{/highlight} in config to set your default model",
  "Override any keybind in {highlight}tui.json{/highlight} via the {highlight}keybinds{/highlight} section",
  "Set any keybind to {highlight}none{/highlight} to disable it completely",
  "Configure local or remote MCP servers in the {highlight}mcp{/highlight} config section",
  "Add {highlight}.md{/highlight} files to {highlight}.exos-agent/commands/{/highlight} for reusable prompts",
  "Use {highlight}$ARGUMENTS{/highlight}, {highlight}$1{/highlight}, {highlight}$2{/highlight} in custom commands for dynamic input",
  "Use backticks to inject shell output (e.g., {highlight}`git status`{/highlight})",
  "Add {highlight}.md{/highlight} files to {highlight}.exos-agent/agents/{/highlight} for specialized AI personas",
  ...
  "Create {highlight}.ts{/highlight} files in {highlight}.exos-agent/tools/{/highlight} to define new LLM tools",
  "Tool definitions can invoke scripts written in Python, Go, etc",
  "Add {highlight}.ts{/highlight} files to {highlight}.exos-agent/plugins/{/highlight} for event hooks",
  "Use plugins to send OS notifications when sessions complete",
  "Create a plugin to prevent Exos Agent from reading sensitive files",
  "Use {highlight}exos-agent run{/highlight} for non-interactive scripting",
  "Use {highlight}exos-agent --continue{/highlight} to resume the last session",
  "Use {highlight}exos-agent run -f file.ts{/highlight} to attach files via CLI",
  "Use {highlight}--format json{/highlight} for machine-readable output in scripts",
  "Run {highlight}exos-agent serve{/highlight} for headless API access to Exos Agent",
  "Use {highlight}exos-agent run --attach{/highlight} to connect to a running server",
  "Run {highlight}exos-agent upgrade{/highlight} to update to the latest version",
  "Run {highlight}exos-agent auth list{/highlight} to see all configured providers",
  "Run {highlight}exos-agent agent create{/highlight} for guided agent creation",
  "Use {highlight}/exos-agent{/highlight} in GitHub issues/PRs to trigger AI actions",
  "Run {highlight}exos-agent github install{/highlight} to set up the GitHub workflow",
  "Comment {highlight}/exos-agent fix this{/highlight} on issues to auto-create PRs",
  "Comment {highlight}/oc{/highlight} on PR code lines for targeted code reviews",
  'Use {highlight}"theme": "system"{/highlight} to match your terminal\'s colors',
  "Create JSON theme files in {highlight}.exos-agent/themes/{/highlight} directory",
  ...
  "Run {highlight}exos-agent debug config{/highlight} to troubleshoot configuration",
  ...
  "Run {highlight}docker run -it --rm ghcr.io/anomalyco/exos-agent{/highlight} in a container",
  "Use {highlight}/connect{/highlight} with Exos Agent Zen for curated, tested models",
