...
export const EXOS_AGENT_BASE_MODE = "base"
export const COMMAND_PALETTE_COMMAND = "command.palette.show"

const EXOS_AGENT_MODE_KEY = "exos-agent.mode"

export const ExosAgentKeymapProvider = KeymapProvider
export const useExosAgentKeymap = useKeymap

export { useBindings, useKeymapSelector }

export type OpenTuiKeymap = ReturnType<typeof useKeymap>
type ExosAgentModeStack = ReturnType<typeof createExosAgentModeStack>
...
const modeStacks = new WeakMap<OpenTuiKeymap, ExosAgentModeStack>()

function isVisiblePaletteCommand(command: Command) {
  return command.hidden !== true && command.name !== COMMAND_PALETTE_COMMAND
}

export function createExosAgentModeStack(keymap: OpenTuiKeymap) {
  keymap.setData(EXOS_AGENT_MODE_KEY, EXOS_AGENT_BASE_MODE)

  const offFields = keymap.registerLayerFields({
    mode(value, ctx) {
      ctx.require(EXOS_AGENT_MODE_KEY, value)
    },
  })
  ...
  const update = () => {
    keymap.setData(EXOS_AGENT_MODE_KEY, stack.at(-1)?.mode ?? EXOS_AGENT_BASE_MODE)
  }

  const stackApi = {
    current() {
      return stack.at(-1)?.mode ?? EXOS_AGENT_BASE_MODE
    },
    push(mode: string) {
      if (disposed) return () => {}
    ...
    dispose() {
      ...
      stack.length = 0
      offFields()
      keymap.setData(EXOS_AGENT_MODE_KEY, undefined)
...
export function useExosAgentModeStack() {
  return getExosAgentModeStack(useExosAgentKeymap())
}

export function getExosAgentModeStack(keymap: OpenTuiKeymap) {
  const value = modeStacks.get(keymap)
  if (!value) throw new Error("ExosAgent mode stack is not registered for this keymap")
  return value
}
...
export function registerExosAgentKeymap(keymap: OpenTuiKeymap, renderer: CliRenderer, config: ResolvedKeymapConfig) {
  const modeStack = createExosAgentModeStack(keymap)
...
export function useCommandSlashes(): Accessor<readonly CommandSlashEntry[]> {
  const keymap = useExosAgentKeymap()
