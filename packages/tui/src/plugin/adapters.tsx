import type { TuiDialogSelectOption, TuiPluginApi, TuiSlotProps } from "@exos-agent-ai/plugin/tui"
...
import type { useExosAgentKeymap } from "../keymap"
...
type Input = {
  ...
  keymap: ReturnType<typeof useExosAgentKeymap>
...
export function createTuiApiAdapters(input: Input): Omit<TuiPluginApi, "lifecycle"> {
  return {
    ...
    mode: {
      current() {
        return Keymap.getExosAgentModeStack(input.keymap).current()
      },
      push(mode) {
        return Keymap.getExosAgentModeStack(input.keymap).push(mode)
      },
    },
    route: {
      register(list) {
