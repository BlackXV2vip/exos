...
import { getExosAgentModeStack, EXOS_AGENT_BASE_MODE, ExosAgentKeymapProvider, registerExosAgentKeymap } from "../src/keymap"
...
test("legacy page key aliases compile as page keys", async () => {
  ...
  function Harness() {
    ...
    const offKeymap = registerExosAgentKeymap(keymap, renderer, config)
...
test("mode-less bindings stay active when exos-agent mode changes", async () => {
  ...
  function Harness() {
    ...
    const popQuestion = getExosAgentModeStack(keymap).push("question")
    counts.question = activeCounts()
    popQuestion()
    const popAutocomplete = getExosAgentModeStack(keymap).push("autocomplete")
