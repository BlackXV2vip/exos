# Exos Agent GitLab Auth

A secure OAuth 2.0 and Personal Access Token authentication plugin for [Exos Agent](https://exos-agent.ai) that enables seamless integration with GitLab.com and self-hosted GitLab instances.

[![npm version](https://img.shields.io/npm/v/@gitlab/exos-gitlab-auth.svg)](https://www.npmjs.com/package/@gitlab/exos-gitlab-auth)
...
- **Automatic Token Refresh** - Seamless token renewal via Exos Agent's auth system
...
- npm 9.0.0 or higher
- Exos Agent installed and configured

### Install via npm

```bash
npm install @gitlab/exos-gitlab-auth
```
...
git clone https://gitlab.com/gitlab-org/editor-extensions/exos-gitlab-auth.git
cd exos-gitlab-auth
...
# In your Exos Agent installation, link the plugin
npm link @gitlab/exos-gitlab-auth
```

## Quick Start

### 1. Configure Exos Agent

Add the plugin to your Exos Agent configuration:

```json
{
  "plugins": ["@gitlab/exos-gitlab-auth"]
}
```
...
Start Exos Agent and use the `/connect` command:
...
1. **Authorization Request**: Exos Agent opens your browser to GitLab's authorization page
...
5. **Secure Storage**: Tokens are stored in `~/.local/share/exos-agent/auth.json` with 600 permissions

#### OAuth Flow Diagram

```mermaid
sequenceDiagram
    participant Exos Agent
    participant Browser
    participant GitLab
    participant LocalServer as Local Callback Server
    participant Storage as Secure Storage

    Exos Agent->>Browser: 1. Open Authorization URL<br/>(with PKCE challenge)
    ...
    LocalServer->>Exos Agent: 8. Authentication Complete
```
...
4. Enter it when prompted by Exos Agent
...
- **Linux/macOS**: `~/.local/share/exos-agent/auth.json`
- **Windows**: `~/.exos-agent/auth.json`
- **Custom**: `$XDG_DATA_HOME/exos-agent/auth.json`
...
- **Linux/macOS**: `~/.local/share/exos-agent/log/gitlab-auth.log`
- **Windows**: `~/.exos-agent/log/gitlab-auth.log`
...
The main plugin that integrates with Exos Agent's authentication system.

**Key Functions:**

- `gitlabAuthPlugin()` - Main plugin export implementing Exos Agent's `Plugin` interface
...
flowchart TD
    ...
    N --> O[Return success to Exos Agent]
```
...
```
~/.local/share/exos-agent/auth.json
{
  "gitlab": {
    "type": "oauth",
...
```typescript
import gitlabAuthPlugin from '@gitlab/exos-gitlab-auth';
...
### Exos Agent Integration

The plugin implements Exos Agent's `Plugin` interface:
...
- Browser shows success but Exos Agent shows failure
...
4. Check debug logs:
   ...
   ```bash
   tail -f ~/.local/share/exos-agent/log/gitlab-auth.log
   ```
...
# View last 50 lines
tail -n 50 ~/.local/share/exos-agent/log/gitlab-auth.log

# Search for errors
grep -i error ~/.local/share/exos-agent/log/gitlab-auth.log
```
...
1. **Check existing issues**: [GitLab Issues](https://gitlab.com/gitlab-org/editor-extensions/exos-gitlab-auth/-/issues)
2. **Review debug logs**: `~/.local/share/exos-agent/log/gitlab-auth.log`
3. **Open a new issue**: Include logs and steps to reproduce
4. **Community support**: Exos Agent Discord/Slack
...
Contributions are welcome! Please see our [Contributing Guide](https://gitlab.com/gitlab-org/editor-extensions/exos-gitlab-auth/-/blob/main/CONTRIBUTING.md) for detailed guidelines on:
...
3. **Testing**: Manual testing via `npm link` and Exos Agent integration
...
**A:** In `~/.local/share/exos-agent/auth.json` with 600 permissions (owner only).
...
**A:** No, authentication requires internet access to GitLab. However, once authenticated, Exos Agent may cache some data.
...
Copyright (c) 2025 Exos Agent GitLab Auth Contributors
...
- Built for [Exos Agent](https://exos-agent.ai)

## Links

- [GitLab Repository](https://gitlab.com/gitlab-org/editor-extensions/exos-gitlab-auth)
- [npm Package](https://www.npmjs.com/package/@gitlab/exos-gitlab-auth)
- [Issue Tracker](https://gitlab.com/gitlab-org/editor-extensions/exos-gitlab-auth/-/issues)
- [Contributing Guide](https://gitlab.com/gitlab-org/editor-extensions/exos-gitlab-auth/-/blob/main/CONTRIBUTING.md)
- [Changelog](https://gitlab.com/gitlab-org/editor-extensions/exos-gitlab-auth/-/blob/main/CHANGELOG.md)
- [Agent Guidelines](https://gitlab.com/gitlab-org/editor-extensions/exos-gitlab-auth/-/blob/main/AGENTS.md)

## Acknowledgments

This project is built for:

- [Exos Agent](https://exos-agent.ai)
- [GitLab Duo](https://about.gitlab.com/gitlab-duo/)
- [GitLab OAuth](https://docs.gitlab.com/ee/api/oauth2.html)

---

**Made with ❤️ for the Exos Agent community**
