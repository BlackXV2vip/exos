...
// This will NOT work with Exos Agent's local HTTP callback server.
...
export function validateInstanceUrl(value) {
    ...
    try {
        ...
        return 'Enter a valid URL, e.g. https://gitlab.com';
    }
}
/**
...
function debugLog(message, data) {
    try {
        ...
        const logDir = path.join(homeDir, '.local', 'share', 'exos-agent', 'log');
...
/**
 * Get Exos Agent auth file path
 */
function getAuthPath() {
    const homeDir = os.homedir();
    const xdgDataHome = process.env.XDG_DATA_HOME;
    if (xdgDataHome) {
        return path.join(xdgDataHome, 'exos-agent', 'auth.json');
    }
    if (process.platform !== 'win32') {
        return path.join(homeDir, '.local', 'share', 'exos-agent', 'auth.json');
    }
    return path.join(homeDir, '.exos-agent', 'auth.json');
}
/**
 * Save OAuth auth data to Exos Agent's auth.json
 * Workaround for Exos Agent not saving the enterpriseUrl field
 */
...
/**
 * Save PAT auth data to Exos Agent's auth.json
 * Workaround for Exos Agent not saving the enterpriseUrl field for API keys
 */
    ...
    const authHook = {
        ...
        async loader(auth) {
            ...
            if (authData.type === 'api') {
                ...
                // 1. metadata.instanceUrl — written by exos-agent when the PAT auth flow
                //    collects the instance URL prompt (exos-agent's `api` auth schema only
                    ...
                    return {
                        ...
                        async callback() {
                            ...
                            try {
                                ...
                                // Save auth data (workaround for Exos Agent not saving enterpriseUrl)
                                ...
                                return {
                                    ...
                                    // correct GitLab instance. exos-agent's OAuth schema includes
            ...
            },
            {
                ...
                async authorize(inputs) {
                    ...
                    try {
                        ...
                        // Save PAT auth data with enterpriseUrl (workaround for Exos Agent not saving it)
                        ...
                        return {
                            ...
                            key: token,
                            provider: normalizedUrl,
                            // Persist the instance URL in metadata (exos-agent's `api` auth
