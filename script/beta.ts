...
const model = "exos-agent/gpt-5.3-codex"
...
async function build() {
  ...
  try {
    await $`./script/build.ts --single`.cwd("packages/exos-agent")
    return true
  } catch (err) {
...
async function fix(pr: PR, files: string[], prs: PR[], applied: number[], idx: number) {
  console.log(`  Trying to auto-resolve ${files.length} conflict(s) with exos-agent...`)
  ...
  try {
    await $`exos-agent run -m ${model} ${prompt}`
  } catch (err) {
    console.log(`  exos-agent failed: ${err}`)
    return false
  }
  ...
  console.log("  Conflicts resolved with exos-agent")
  return true
}
...
async function smoke(prs: PR[], applied: number[]) {
  ...
  console.log("\nTrying to fix final smoke check with exos-agent...")
  ...
  const prompt = [
    ...
    "Run `./script/build.ts --single` in `packages/exos-agent`.",
