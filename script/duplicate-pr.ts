...
import { createExosAgent } from "@exos-agent-ai/sdk"
...
`)
  ...
  const exos-agent = await createExosAgent({ port: 0 })
  ...
  try {
    ...
    const session = await exos-agent.client.session.create()
    const result = await exos-agent.client.session
      .prompt({
