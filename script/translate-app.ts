...
export function parseTranslationArgs(args: string[]) {
  const parsed = parseArgs({
    ...
    options: {
      ...
      model: { type: "string", default: "exos-agent/gpt-5.5" },
...
export function glossaryFile(locale: Locale) {
  if (locale === "zh") return ".exos-agent/glossary/zh-cn.md"
  if (locale === "zht") return ".exos-agent/glossary/zh-tw.md"
  return `.exos-agent/glossary/${locale}.md`
}
...
export function sessionIDFromEvents(output: string) {
  ...
  if (!match?.[1]) throw new Error("Exos Agent did not report a session ID.")
  return match[1]
}

export function sessionModels(value: unknown) {
  if (!isRecord(value) || !Array.isArray(value.messages))
    throw new Error("Exos Agent returned an invalid session export.")
  return value.messages.flatMap((message) => {
    ...
    if (typeof message.info.providerID !== "string" || typeof message.info.modelID !== "string") {
      throw new Error("Exos Agent session export omitted the assistant model.")
    }
    return [
      {
...
Options:
  -c, --concurrency <count>  Maximum parallel Exos Agent runs for 'all' (default: 4)
      --model <provider/id>  Exos Agent model (default: exos-agent/gpt-5.5)
      --variant <name>       Model variant (default: xhigh)
      --dry-run              Report drift without running Exos Agent
...
`)
  ...
  if (failed.length) console.error(`\nExos Agent failed for: ${failed.map((result) => result.locale).join(", ")}`)
...
async function translate(
  ...
  env.EXOS_AGENT_DISABLE_PROJECT_CONFIG = "1"
  env.EXOS_AGENT_CONFIG_CONTENT = JSON.stringify(
    translationConfig(
      agent,
      model,
  ...
  const exported = Bun.spawn(["exos-agent", "--pure", "export", sessionID, "--sanitize"], {
    cwd: root,
    env,
    stdout: "pipe",
    stderr: "pipe",
...
async function resolveModelVariant(model: string, variant: string) {
  ...
  const env = isolatedEnvironment()
  env.EXOS_AGENT_DISABLE_PROJECT_CONFIG = "1"
  const proc = Bun.spawn(["exos-agent", "--pure", "models", provider, "--verbose"], {
    cwd: root,
    env,
    stdin: "ignore",
    stdout: "pipe",
...
function isolatedEnvironment() {
  ...
  delete env.EXOS_AGENT_CONFIG
  delete env.EXOS_AGENT_CONFIG_DIR
  delete env.EXOS_AGENT_CONFIG_CONTENT
  delete env.EXOS_AGENT_PERMISSION
  delete env.EXOS_AGENT_AUTO_SHARE
  return env
}
