# Remove `packages/exos-agent/src/storage/db.ts`

## Goal

Remove all production usages of the legacy `packages/exos-agent/src/storage/db.ts` module.
...
This does not mean removing SQLite or Drizzle everywhere in one step. The smaller target is deleting the exos-agent legacy wrapper by moving call sites onto deeper modules or onto the core/effect database adapter directly.

## Current Inventory

Production imports from `packages/exos-agent/src/storage/db.ts` are concentrated in 22 source files:

- `packages/exos-agent/src/account/repo.ts`
- `packages/exos-agent/src/cli/cmd/db.ts`
- `packages/exos-agent/src/cli/cmd/import.ts`
- `packages/exos-agent/src/cli/cmd/stats.ts`
- `packages/exos-agent/src/control-plane/workspace.ts`
- `packages/exos-agent/src/index.ts`
- `packages/exos-agent/src/node.ts`
- `packages/exos-agent/src/permission/index.ts`
- `packages/exos-agent/src/project/project.ts`
- `packages/exos-agent/src/server/projectors.ts`
- `packages/exos-agent/src/server/routes/instance/httpapi/handlers/sync.ts`
- `packages/exos-agent/src/server/shared/fence.ts`
- `packages/exos-agent/src/session/message-v2.ts`
- `packages/exos-agent/src/session/projectors.ts`
- `packages/exos-agent/src/session/prompt.ts`
- `packages/exos-agent/src/session/session.ts`
- `packages/exos-agent/src/session/todo.ts`
- `packages/exos-agent/src/share/share-next.ts`
- `packages/exos-agent/src/storage/db.ts`
- `packages/exos-agent/src/sync/index.ts`
- `packages/exos-agent/src/worktree/index.ts`
...
Status: Completed. Startup, the public node export, and database CLI tooling no longer import the legacy exos-agent database wrapper; `packages/exos-agent/src/storage/db.ts` has been deleted.
...
- Move database path and client startup behind the core/effect database module rather than the exos-agent wrapper.
...
Status: Completed. `SyncEvent` and the exos-agent projector boundary were removed; session/message event projection now lives in core EventV2/projector infrastructure.
...
Status: Completed. These services no longer import the legacy exos-agent database wrapper.
...
Status: Completed. Session/message reads and projector writes have moved off the legacy exos-agent database wrapper.
...
Status: Completed. Remaining one-off CLI/admin reads and writes now use core database services or domain services instead of the legacy exos-agent database wrapper.
...
All migration groups are complete or superseded. `packages/exos-agent/src/storage/db.ts` has been deleted.

## Superseded: Data Migrations

Status: Superseded. No exos-agent data-migration group remains.

The previous exos-agent `data-migration.ts` service only backfilled session usage from message rows. That work is now covered by core database migration `packages/core/src/database/migration/20260510033149_session_usage.ts`, so there is no separate exos-agent data-migration group.
...
- Existing schema ownership remains in `packages/core/src/**/*.sql.ts`; do not move table definitions back into `packages/exos-agent`.

## Verification Commands

- `rg "@/storage/db|./storage/db|Database\.(use|transaction|effect|Client|getPath)|\bTxOrDb\b|\bTransaction\b" packages/exos-agent/src`
- `bun typecheck` from `packages/exos-agent`
- Relevant package tests from `packages/exos-agent`, not the repo root
