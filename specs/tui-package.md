...
Move the canonical Exos Agent terminal application from
`packages/exos-agent/src/cli/cmd/tui` into a self-contained workspace package while
...
```text
packages/exos-agent ---\
                      > @exos-agent-ai/tui -> @exos-agent-ai/sdk
packages/cli --------/
```
...
generic presentation libraries. It must not depend on `packages/exos-agent`,
`packages/cli`, or `@exos-agent-ai/core`.

The SDK is the TUI's Exos Agent boundary. Missing backend data or operations must
...
- Do not preserve private imports by creating aliases from `packages/tui` back
  into `packages/exos-agent`.
- Do not replace private `packages/exos-agent` imports with `@exos-agent-ai/core`
...
- Keep legacy CLI command parsing, server startup, worker management,
  authentication, and config discovery outside `@exos-agent-ai/tui`.

## Ownership Boundary

### `@exos-agent-ai/tui` Owns
...
- Exos Agent domain data displayed by the TUI
...
```text
packages/exos-agent/src/cli/cmd/tui
```

Its private dependency on `packages/exos-agent` is primarily expressed through
the `@/*` TypeScript alias, which resolves to `packages/exos-agent/src/*`.
...
Status: Completed. The private `@exos-agent-ai/tui` workspace package now has an
...
- Add `packages/tui/package.json` with the name `@exos-agent-ai/tui`.
...
- The package has no dependency on `exos-agent`, `@exos-agent-ai/cli`, or
  `@exos-agent-ai/core`.
...
primitives now live in `@exos-agent-ai/tui`. The legacy host consumes explicit
...
and build inputs through `@exos-agent-ai/tui/runtime`. Movable app, component,
route, and feature-plugin code no longer reads Exos Agent globals or process state;
...
Replace process-global Exos Agent state with resolved TUI inputs.
...
`@exos-agent-ai/tui/config` now owns schemas, defaults, keybind resolution, the
...
- Keep config path discovery, project/global precedence, migration, variable
  expansion, and plugin package installation in `packages/exos-agent` initially.
...
canonical in `@exos-agent-ai/tui`. Configured references resolve through the new
...
adapters rather than Exos Agent domain implementations.

Make the SDK the only Exos Agent domain boundary used by the TUI.
...
- Domain-facing TUI code imports Exos Agent data and operations only from
  `@exos-agent-ai/sdk`.
...
`@exos-agent-ai/tui`. The legacy host injects a narrow plugin host that retains
...
- Move plugin rendering tests to `packages/tui`; retain installation/loading
  integration tests in `packages/exos-agent`.
...
- The package has no imports from `packages/exos-agent`, `packages/cli`, or
  `@exos-agent-ai/core`.
...
public `@exos-agent-ai/tui` root while retaining worker/server/config/plugin and
...
- Keep the legacy yargs commands corresponding to current `thread.ts` and
  `attach.ts` in `packages/exos-agent`.
- Keep the legacy embedded worker and server startup in `packages/exos-agent`.
- Change those adapters to load config, create transport inputs, and call the
  public `@exos-agent-ai/tui` API.
...
- Remove obsolete `@tui/*` path mappings from `packages/exos-agent`.
- Remove stale test fixtures and update all imports to package exports.
- Narrow `@exos-agent-ai/tui` exports to intentional public entrypoints.
...
- No source under `packages/tui` imports `@/...`, `@exos-agent-ai/core`, or either
  executable package.
...
- TUI package imports do not reach into executable or backend implementation
  packages.
- SDK wire data is treated as the source of truth for Exos Agent domain state.
...
cd packages/exos-agent && bun typecheck
...
rg '@exos-agent-ai/core|packages/exos-agent|packages/cli' packages/tui
rg 'src/cli/cmd/tui|@tui/' packages/exos-agent/src packages/exos-agent/test
```
...
- Build the current-platform `packages/exos-agent` binary.
