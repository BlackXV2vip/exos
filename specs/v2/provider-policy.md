...
Both `action` and `resource` use exos-agent's existing wildcard matching behavior.
...
The relative policy precedence of direct project files and `.exos-agent` files is intentionally deferred until `.exos-agent` configuration is reviewed.
...
Plugins must not be allowed to add, remove, or override policy statements. Plugins can contribute functionality or configured providers; policy determines whether exos-agent permits an operation through its managed execution paths.
