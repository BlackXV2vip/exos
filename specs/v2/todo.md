...
The exos-agent server has moved to the Effect HttpApi backend. Remaining work is
...
- remove the public in-memory `@exos-agent-ai/llm` tool loop after replacing its
...
- plugins get global "exos-agent" instance like in that post i showed
- exos-agent instance has stuff like `exos-agent.session.prompt()` or
  `exos-agent.tool.register({...})`
...
- decide whether to preserve deprecated `@exos-agent-ai/llm` orchestration exports
